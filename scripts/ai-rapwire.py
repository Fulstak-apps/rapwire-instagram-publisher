...
Return ONLY JSON: {{"index":number,"headline":string,"story":string,"caption":string,"visual_scene":string,"source_label":string,"featured_person":string,"instagram_handle":string,"instagram_profile_url":string,"extra_context":string,"content_format":"photo_news|tweet_statement|video_repost"}}.
Headline must be factual and premium-newsroom clean. Story must be 110-180 informative words in complete sentences and explain what happened, why it matters, timeline/status, and key verified context. Never choose a ranking/list headline unless the story includes every item promised by that headline. If the event is a tweet, statement, court exhibit or social-text post, set content_format to tweet_statement: preserve the readable original statement with no decorative boxes, no overlaid headline, and no source-blog handle in the graphic. If the source is a playable trailer, music video, interview or Reel, set content_format to video_repost: download/repost the video rather than using a screenshot. Otherwise set photo_news. visual_scene must describe the specific moment shown by the selected source/event photo, not an invented or abstract scene. Verify the featured person's current official Instagram account with web search; never infer the handle from the stage name. Include all necessary verified context in story or extra_context; the renderer will add as many carousel pages as readability requires. Never leave a sentence unfinished or use placeholder copy.
CANDIDATES:\n{text}'''
